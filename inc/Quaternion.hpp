...
public:
    ...
    Quaternion(const Eigen::Vector4d& q) : _q(q) {Normalize();}
    ~Quaternion() = default;
    ...
    Eigen::Matrix<double, 4, 3> GetRightMultiplicationMatrix() const; // Eq. 2.88

private:
    Eigen::Vector4d _q; // Quaternion components: [q1, q2, q3, q4], q4 being the scalar part
    Eigen::Matrix<double, 4, 3> GetLeftMultiplicationMatrix() const; // Eq. 2.87
};